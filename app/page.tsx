...
const categories = [
  { name: "Project Management", icon: "📋", count: 42 },
  { name: "Email Marketing", icon: "📧", count: 38 },
  { name: "Web Hosting", icon: "🌐", count: 31 },
  { name: "Design Tools", icon: "🎨", count: 27 },
  { name: "CRM", icon: "🤝", count: 25 },
  { name: "Analytics", icon: "📊", count: 19 },
];

const reviews = [
  { title: "Notion vs Obsidian: Which Note-Taking App Wins in 2024?", excerpt: "We tested both apps for 30 days across teams of different sizes. Here's what we found.", date: "Dec 12, 2024", readTime: "8 min read", tag: "Comparison" },
  { title: "Best Email Marketing Tools for E-commerce in 2024", excerpt: "A comprehensive review of Mailchimp, Klaviyo, ActiveCampaign and more for online stores.", date: "Dec 8, 2024", readTime: "12 min read", tag: "Roundup" },
  { title: "Webflow Review 2024: Is It Worth the Price?", excerpt: "After building 50+ sites on Webflow, here's our honest take on its pros, cons and pricing.", date: "Dec 3, 2024", readTime: "10 min read", tag: "Review" },
  { title: "HubSpot Free CRM: Everything You Get (And What You Don't)", excerpt: "The free tier is genuinely powerful, but there are catches. We break down exactly what's included.", date: "Nov 28, 2024", readTime: "7 min read", tag: "Review" },
];

const comparison = [
  { name: "Notion", price: "$8/mo", storage: "Unlimited", api: "✅", ai: "✅", mobile: "✅", offline: "✅" },
  { name: "Asana", price: "$10.99/mo", storage: "Unlimited", api: "✅", ai: "✅", mobile: "✅", offline: "❌" },
  { name: "ClickUp", price: "$7/mo", storage: "Unlimited", api: "✅", ai: "✅", mobile: "✅", offline: "✅" },
];
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <nav className="border-b border-gray-100 sticky top-0 bg-white/95 backdrop-blur z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex gap-6 text-sm text-gray-600 font-medium">
            <a href="#" className="hover:text-indigo-600 transition">Reviews</a>
            <a href="#" className="hover:text-indigo-600 transition">Compare</a>
            <a href="#" className="hover:text-indigo-600 transition">Categories</a>
            <a href="#" className="hover:text-indigo-600 transition">Blog</a>
          </div>
          <button className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-5 py-2.5 rounded-full transition">
            Get Recommendations
          </button>
        </div>
      </nav>
      ...
      <section className="bg-gradient-to-b from-indigo-50 to-white py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex max-w-lg mx-auto gap-3 mb-8">
            <input
              type="text"
            ...
            />
            <button className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-4 rounded-xl font-bold transition whitespace-nowrap">
              Find Tools
            </button>
          </div>
          <div className="flex flex-wrap justify-center gap-2">
            {["Notion", "Figma", "HubSpot", "Webflow", "Mailchimp", "Slack", "Linear"].map((tag) => (
              <button key={tag} className="bg-white border border-gray-200 hover:border-indigo-400 hover:text-indigo-600 text-gray-600 text-sm px-4 py-1.5 rounded-full transition shadow-sm">
                {tag}
              </button>
            ))}
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="flex items-center justify-between mb-8">
          ...
          <a href="#" className="text-indigo-600 hover:text-indigo-700 text-sm font-medium transition">View all reviews →</a>
        </div>
          ...
          {tools.map((tool) => (
            <div key={tool.id} className="border border-gray-100 hover:border-indigo-200 rounded-2xl p-6 transition hover:shadow-lg hover:shadow-indigo-50 group">
              ...
              <div className="flex gap-2">
                <a href="#" className="flex-1 border border-indigo-200 text-indigo-600 hover:bg-indigo-50 text-sm font-semibold py-2.5 rounded-xl text-center transition">
                  Read Review
                </a>
                <a href="#" className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold py-2.5 rounded-xl text-center transition">
                  Visit Site →
                </a>
              </div>
            </div>
      ...
      <section className="bg-gray-50 py-16 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories.map((cat) => (
              <button key={cat.name} className="bg-white border border-gray-100 hover:border-indigo-300 rounded-2xl p-5 text-center transition hover:shadow-md group">
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
            ...
            <tbody>
              {comparison.map((row, i) => (
                <tr key={row.name} className={i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}>
                  <td className="px-5 py-4 font-bold text-indigo-600">{row.name}</td>
        ...
        <div className="mt-4 text-center">
          <a href="#" className="text-indigo-600 hover:text-indigo-700 text-sm font-medium">See full comparison →</a>
        </div>
      </section>
      ...
      <section className="bg-gray-50 py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-4">
            {reviews.map((r) => (
              <a key={r.title} href="#" className="block bg-white border border-gray-100 hover:border-indigo-200 rounded-2xl p-6 transition hover:shadow-md group">
